...
export default (yesHref) => {
    ...
    element.innerHTML += views.replace('{{titlePage}}', '<i class="bi bi-bezier2"></i> Linaje')
        .replace('{{label_jenkins}}', 'Ingresa el pipeline jenkins de linaje.')
...
const behaviors = (parent, yesHref) => {
    parent.querySelector('#continue').addEventListener('click', () => {
        ...
        dataIssue[Vault.groups.linaje.key][Vault.fields.jenkinsUrl.key] = parent.querySelector(`#${Vault.fields.jenkinsUrl.key}`).value.toUpperCase();
        dataIssue[Vault.groups.linaje.key][Vault.fields.jenkinsFolder.key] = parent.querySelector(`#${Vault.fields.layerScript.key}`).value.toUpperCase();
        dataIssue[Vault.groups.linaje.key][Vault.fields.reusePipeline.key] = parent.querySelector(`#${Vault.fields.reusePipeline.key}`).checked;
