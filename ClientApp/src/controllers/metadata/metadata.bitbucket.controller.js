...
export default (yesHref, noHref) => {
    ...
    element.innerHTML = noHref ? viewsChoose.replace('{{titleChoose}}', '&#191;Quieres agregar metadata?').replace('{{idPanelPrincipal}}', 'bitbucketForm') : '';

    element.innerHTML += views.replace('{{titlePage}}', '<i class="bi bi-table"></i> Metadata')
        .replace('{{label_bitbucket}}', 'Ingresa la url de bitbucket de metadata.')
...
const behaviors = (parent, yesHref, noHref) => {
    ...
    parent.querySelector('#continue').addEventListener('click', () => {
        ...
        dataIssue[Vault.groups.metadata.key] = {};
        dataIssue[Vault.groups.metadata.key][Vault.groups.metadata.key] = true;
        dataIssue[Vault.groups.metadata.key][Vault.fields.bitbucketUrl.key] = bitbucketParts[0];
        dataIssue[Vault.groups.metadata.key][Vault.fields.bitbucketProject.key] = bitbucketParts[1];
        dataIssue[Vault.groups.metadata.key][Vault.fields.bitbucketRepository.key] = bitbucketParts[2];
