...
export default (yesHref) => {
    ...
    element.innerHTML = views.replace('{{titlePage}}', '<i class="bi bi-server"></i> Generar scripts de backup')
        .replace('{{label_bitbucket}}', 'Ingresa la url de bitbucket donde se generar&aacute;n los scripts.')
...
const behaviors = (parent, yesHref) => {
    ...
    parent.querySelector('#continue').addEventListener('click', () => {
        ...
        dataIssue[Vault.groups.generatorBackup.key][Vault.fields.bitbucketUrl.key] = bitbucketParts[0];
        dataIssue[Vault.groups.generatorBackup.key][Vault.fields.bitbucketProject.key] = bitbucketParts[1];
        dataIssue[Vault.groups.generatorBackup.key][Vault.fields.bitbucketRepository.key] = bitbucketParts[2];
