...
export default (yesHref) => {
    ...
    element.innerHTML = views.replace('{{titlePage}}', '<i class="bi bi-train-freight-front"></i> Motor Minsait - Extracción RDV')
        ...
        .replace('{{label_taxonomia_script}}', '2. Taxonom&iacute;a data:')
        ...
        .replaceAll('{{script_label}}', 'Carpeta data')
        .replaceAll('{{script_path_label}}', 'data')
        .replaceAll('{{default_value_scripts}}', 'data');

    element.querySelector(`#${Vault.fields.layerTemporary.key}`).innerHTML = '<option>RDV</option>';
    element.querySelector(`#${Vault.fields.layerScript.key}`).innerHTML = '<option>RDV</option>';
...
const behaviors = (parent, yesHref) => {
    parent.querySelector('#continue').addEventListener('click', () => {
        ...
        dataIssue[Vault.groups.process.key][Vault.fields.extractionEngine.key] = true;
